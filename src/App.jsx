  ...
  return <motion.div style={{display:"flex",overflow:'hidden',...props.style}} ref={ref}>
      {
          [...props.text].map((el,index)=>{
              return <motion.div  initial={{y:400,scaleX:0}} animate={isInView ?{y:0,scaleX:1} :{y:400}} transition={{delay:index * 0.05,duration:1}} key={index}>
                  {el}
              </motion.div>
          })
      }
    ...
    <motion.div className="App">
      ...
      <motion.div className='section-1'>
        ...
        <motion.div className='sec-1-a'>
          <motion.p className='header'><AnimateLetterInViewHidden text="BEST"/>INVESTISSMENT <br/>DON'T <span className='bold'>WAIT</span></motion.p>
          <motion.p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s.</motion.p>
        </motion.div>

        <motion.div initial={{scale:0}} transition={{delay:1,duration:.3}} animate={{scale:1}} className='sec-2-a'>
          <Tokens/>
        </motion.div>
